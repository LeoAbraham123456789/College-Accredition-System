...
public class Ec extends JFrame implements ActionListener
{
	/**
	 * 
	 */
	...
	public Ec()
	{
		super("Electronics and Communication Engineering");
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(arg0.getActionCommand()=="Update")
		{
			...
			{	Class.forName("com.mysql.cj.jdbc.Driver");
				...
				if(reg.isSelected())
				{
					...
					stmt.execute("UPDATE EC SET status=1 WHERE topics='Registration'");
					
				}
				else
				{
					stmt.execute("UPDATE EC SET status=0 WHERE topics='Registration'");
					//stmt.execute("UPDATE progress_bar SET percentage=percentage-33 WHERE Dept='CS'");
				}
				if(yr.isSelected())
				{
					
					stmt.execute("UPDATE EC SET status=1 WHERE topics='Yearly Report'");
					
				}
				else
				{
					stmt.execute("UPDATE EC SET status=0 WHERE topics='Yearly Report'");
					//stmt.execute("UPDATE progress_bar SET percentage=percentage-33 WHERE Dept='CS'");
				}
				if(ssr.isSelected())
				{
					
					stmt.execute("UPDATE EC SET status=1 WHERE topics='SSR'");
					//stmt.execute("UPDATE progress_bar SET percentage+=34 WHERE Dept='CS'");
				}
				else
				{
					stmt.execute("UPDATE EC SET status=0 WHERE topics='SSR'");
				...
				{
					stmt.execute("UPDATE progress_bar SET percentage=100 WHERE Dept='EC'");
				}
				else if((reg.isSelected()&&yr.isSelected())||(yr.isSelected()&&ssr.isSelected())||(reg.isSelected()&&ssr.isSelected()))
					stmt.execute("UPDATE progress_bar SET percentage=66 WHERE Dept='EC'");
				else if(reg.isSelected()||yr.isSelected()||ssr.isSelected())
					stmt.execute("UPDATE progress_bar SET percentage=33 WHERE Dept='EC'");
				else
					stmt.execute("UPDATE progress_bar SET percentage=0 WHERE Dept='EC'");
				con.close();
		...
		if(arg0.getActionCommand()=="Refresh")
		{
			...
			{	Class.forName("com.mysql.cj.jdbc.Driver");
				Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/miniproject", "root", "Leo@2002");
				Statement stmt= con.createStatement();
				ResultSet rs=stmt.executeQuery("Select * from EC");
				while(rs.next())
				{
